...
class APIClient:
    ...
    async def make_request(
        ...
        # For Gemini, we use the API key as a query parameter
        api_key = sub_provider["api_key"]
        headers = {
            'content-type': 'application/json'
        }
        
        # Convert OpenAI format to Gemini format
        gemini_data = self._convert_to_gemini_format(data)
        
        url = f'{self.config.base_url}/v1beta/models/{data.get("model", "gemini-1.5-flash")}:{endpoint}?key={api_key}'
    ...
    def _convert_to_gemini_format(self, openai_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for message in messages:
            ...
            if role == 'assistant':
                ...
                # System messages can be handled as user messages in Gemini
                gemini_role = 'user'
            else:
                gemini_role = 'user'
...
class ResponseHandler:
    ...
    def create_completion_response(self, response_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        openai_response = self._convert_from_gemini_format(response_data)
        return {
            'provider_id': self.config.provider_id,
            **openai_response
        }

    def _convert_from_gemini_format(self, gemini_data: Dict[str, Any]) -> Dict[str, Any]:
        """Convert Gemini response format to OpenAI format"""
        ...
        return {
            ...
            'model': gemini_data.get('model', 'gemini-1.5-flash'),
            'choices': choices,
            'usage': {
                'prompt_tokens': gemini_data.get('usageMetadata', {}).get('promptTokenCount', 0),
                'completion_tokens': gemini_data.get('usageMetadata', {}).get('candidatesTokenCount', 0),
                'total_tokens': gemini_data.get('usageMetadata', {}).get('totalTokenCount', 0)
            }
        }
...
class StreamHandler:
    ...
    async def handle_stream(
        ...
        async def stream_generator() -> AsyncGenerator[str, None]:
            ...
            # Gemini streaming format is different from OpenAI
            async for line in response.aiter_lines():
                if line.strip():
                    try:
                        # Parse Gemini streaming response
                        gemini_chunk = ujson.loads(line)
                        openai_chunk = self._convert_gemini_chunk_to_openai(gemini_chunk)
                        yield f'data: {ujson.dumps(openai_chunk)}\n\n'
                    except Exception:
                        continue
    ...
    def _convert_gemini_chunk_to_openai(self, gemini_chunk: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'created': int(time.time()),
            'model': gemini_chunk.get('model', 'gemini-1.5-flash'),
            'choices': choices,
            'provider_id': self.config.provider_id
        }
